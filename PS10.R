...
Q1 <- ggplot(ipip.l, aes(x=exer, y=BMI, fill=gender)) +
    geom_boxplot(outlier.size = .8, lwd=0.5) + theme_classic()
Q1
...
ipip.f <- ipip %>%
    filter(logMedInc>10)
Q2b <- ggplot(ipip.f,aes(x=logMedInc,y=BMI, color=gender))+
    geom_point(size=.5,alpha=.4)+
    geom_smooth(method='lm')
Q2b
...
Q3 <- ggplot(ipip.l, aes(x=gender, y=logMedInc, group=exer, colour=exer)) +
    stat_summary(fun.y=mean,  geom="bar", fill=NA, position="dodge") +
    stat_summary(fun.y=mean_cl_boot, geom="errorbar", position=position_dodge(width=.9), width=.3) + 
    coord_cartesian(ylim = c(10, 12)) + theme_classic()
Q3
...
Q5 <- ggplot(ipip.g, aes(x=BMI_cat, y=trait_mean, group=trait, colour=trait)) +
    geom_point() +
    geom_line(aes(linetype='solid')) +
    facet_wrap(vars(gender)) + theme_classic()
Q5
